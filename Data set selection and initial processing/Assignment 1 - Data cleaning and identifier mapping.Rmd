...
[European Nucleotide Archive (ENA)](https://www.ebi.ac.uk/ena/browser/home) gene
symbols. We now check how many genes are annotated with the ENA gene symbols. Upon 
...
is a digit. Since the nomenclature of the 
...
HGNC database. However, it is documented in the ENA database. Futher cross-referencing
...
Since there are `r length(byDots)` genes annotated with the ENA gene symbols, we will
try to map these genes to the HUGO gene symbols.

## Mapping ENA Gene Symbols to HUGO Gene Symbols

```{r, message = FALSE, tidy=TRUE, attr.output='style="max-height: 200px;"'}
# Get the ENA gene symbols of the genes that are not already annotated with the HUGO gene symbols
(enaSymbols <- rownames(cpm_filtered)[byDots])
```
