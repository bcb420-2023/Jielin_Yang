...
```{r message = FALSE, tidy=TRUE, warning=FALSE}
# Interestingly, taking the transpose of the heatmap matrix sorts the sample 
# names. Therefore, to correctly apply the annotation, we need to sort the 
# sample matrix in the same order as the heatmap matrix
...
```{r message = FALSE, tidy=TRUE, warning=FALSE}
model_treat_cul <- model.matrix(~0 + sample_matrix$treatment + sample_matrix$culture)
rownames(model_treat_cul) <- rownames(sample_matrix)

# View the model matrix
DT::datatable(model_treat_cul, rownames = TRUE,
