...
We performed GSEA analysis using the same parameters as the previous analysis,
except that this time we use the `vtp_tgf.rnk` gene list for the analysis. 
...
gsea_vtp_tgf_up <- read.csv("./Data/VTPTGFPreranked/gsea_report_for_na_pos_1680149018385.csv",
    header = TRUE)
gsea_vtp_tgf_down <- read.csv("./Data/VTPTGFPreranked/gsea_report_for_na_neg_1680149018385.csv", 
    header = TRUE)
```
...
the presence of TGF-β1. In summary, ``r nrows(gsea_vtp_tgf_up)`` gene sets are
...
out of ``r nrows(gsea_vtp_tgf_down)`` enriched gene sets are found to be signifiant.
