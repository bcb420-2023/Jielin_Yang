...
## Gene Set Not Significantly Enriched

First, we obtain all gene sets and genes with annotation in the GMT file.
...
annotation. We now examine whether there are any genes in our expression dataset
that are not found to be enriched or annotated.
...
```{r message=FALSE, warning=FALSE, tidy=TRUE, fig.align="center", fig.width=6}
...
# How many genes have no annotation?
length(genes_no_anno)
```

```{r message = FALSE, tidy=TRUE, warning=FALSE}
# How many of those genes are significantly differentially expressed?
sum(dge_vtp_ctrl[genes_no_anno, "FDR"] < 0.05)
```


```{r message = FALSE, tidy=TRUE, warning=FALSE}
# Obtain the set of upregulated genes with no annotation
up_genes_no_anno <- intersect(genes_no_anno, rownames(dge_vtp_ctrl[dge_vtp_ctrl$FDR < 0.05 & dge_vtp_ctrl$logFC > 0, ]))

# List the genes
DT::datatable(dge_vtp_ctrl[up_genes_no_anno, c("logFC", "logCPM", "FDR")],
