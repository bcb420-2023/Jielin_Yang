...
vtp_ctrl_gsea_upreg <- read.csv(
    "./Data/VTPControlPreranked/gsea_report_for_na_pos_1680148985575.csv", 
    header = TRUE)
vtp_ctrl_gsea_downreg <- read.csv(
    "./Data/VTPControlPreranked/gsea_report_for_na_neg_1680148985575.csv", 
    header = TRUE)
```
...
respectively for the up- and down-regualted genes. 
...
gsea_up <- ggplot2::ggplot(upreg_table[1:5,], aes(x = `Enrichment Score`, 
    ...
    geom_bar(stat = "identity", color = "black") + # Define border color
    labs(x = "Enrichment Score", y = NULL) +
    theme_minimal() +
    theme(axis.line.x = element_line(), # Add a solid line for the x-axis
        axis.text.x = element_text(size = 10), # Change the font size of axis labels
...
Interestingly , it has been previously reported in other cell lines
that VTP are associated with an inhibition of oxidative phosphorylation. [@kuramoto2020verteporfin]
This is not expected since an increased mitochondrial activity is associated with
an upregualted energy consuption. Therefore, further analysis on the global pathway
network may allow us to better understand the transcriptional changes during 
this process.
...
**Table 1.** Enriched gene sets in the upregulated genes in the VTP treatment condition.
