...
suppress the YAP/TAZ complex under the maximal strain condition with or without the presence of TGF-β1, a 
...
network analysis to access the enriched pathways and using them to infer the
...
we used in the analysis, including utility packages. [GSEA 4.3.2](https://www.gsea-msigdb.org/gsea/index.jsp) 
and [Cytoscape 3.9.1](https://cytoscape.org/) softwares
with the EnrichmentMap pipeline are also required for the analysis, and they must 
...
We will perform GSEA on the differential gene expression results between the control
...
the genes integrates the sign of fold change change, and
...
This annotation file curates all GO: Biological Process terms as well as
...
```{r message = FALSE, tidy=FALSE, warning=FALSE, eval=FALSE}
# Running GSEA
...
```{r message = FALSE, tidy=FALSE, warning=FALSE, fig.align="center", fig.width=10, fig.height=4}
...
further collaposed the clusters into a single node and applied the `CoSE (Compound Spring Embedder)`
algorithm to visualize global themes of the network. A combined analysis
...
genes in the enrichment results, and there are ``r length(anno_genes)`` genes having
annotation.
...
dataset is curated using a mixture of HGNC symbols and GenBank ID. These IDs are
...
excluded in our analysis. We have separately performed ORA for the up- and down-regualted
genes with no annotation using all available pathways including GO IEA terms.
No enriched terms were found. Details documented in the [jounral](https://github.com/bcb420-2023/Jielin_Yang/wiki/Gene-Set-Enrichment-and-Network-Analysis#genes-with-no-annotation).
