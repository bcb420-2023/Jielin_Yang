...
vtp_ctrl_gsea_upreg <- read.csv(
    ...
    header = TRUE)
vtp_ctrl_gsea_downreg <- read.table(
    "./Data/VTPControlPreranked/gsea_report_for_na_neg_1680148985575.csv", 
    header = TRUE)
```


Our GSEA analysis identified `2282` out of `5256` specified gene sets associated
with the upregulated genes in the VTP treatment condition. Of these, `503` gene 
sets were significantly enriched (FDR < 0.05) with an average enrichment score
